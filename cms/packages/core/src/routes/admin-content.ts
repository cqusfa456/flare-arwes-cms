    ...
    const previewHTML = `
      ...
      <body>
        ...
        <div class="meta">
          ...
          <strong>${t('Status:')}</strong> ${formData.get('status') || 'draft'}<br>
    ...
    const previewHTML = `
      ...
      <body>
        <div class="meta">
          ...
          <strong>${t('Created:')}</strong> ${new Date(versionData.created_at).toLocaleString()}<br>
        ...
        ${data.excerpt ? `<h3>${t('Excerpt:')}</h3><p>${data.excerpt}</p>` : ''}
