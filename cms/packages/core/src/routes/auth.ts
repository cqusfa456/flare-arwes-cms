...
authRoutes.post('/seed-admin',
  ...
  try {
    ...
    const existingAdmin = await db.prepare('SELECT id FROM users WHERE email = ? OR username = ?')
      .bind('admin@arwes.dev', 'admin')
      .first()

    if (existingAdmin) {
      // Update the password to ensure it's correct for testing
      const passwordHash = await AuthManager.hashPassword('arwes-admin!')
      await db.prepare('UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?')
    ...
    const now = Date.now()
    const adminEmail = 'admin@arwes.dev'.toLowerCase()
